...
rgbeLoader.load(
  ...
  (envMAap) => {
    //设置球形贴图
    envMAap.mapping = THREE.EquirectangularReflectionMapping;
    scene.background = envMAap;
    scene.environment = envMAap;
    planeMaterial.envMap = envMAap;
  }
);
...
gui.add(planeMaterial, "aoMapIntensity").min(1).max(10).name("ao透明强度");
